...
export default function MemberGymRegister() {
  ...
  // 가상 헬스장 데이터 (검색 시뮬레이션용)
  const mockGyms = [
    { id: 1, name: "짐바로핏 강남 1호점", address: "서울 강남구 테헤란로 123", distance: "300m" },
    { id: 2, name: "짐바로핏 역삼 2호점", address: "서울 강남구 논현로 456", distance: "1.2km" },
    { id: 3, name: "에이블짐 홍대점", address: "서울 마포구 양화로 789", distance: "5.5km" },
    { id: 4, name: "스포애니 판교역점", address: "경기 성남시 분당구 판교로 101", distance: "12km" },
  ];
  ...
  const handleSearch = () => {
    ...
    setTimeout(() => {
      ...
      const filtered = mockGyms.filter((gym) =>
        gym.name.includes(keyword) || gym.address.includes(keyword)
      );
  ...
  const handleRegisterConfirm = () => {
    ...
    // TODO: 서버로 등록 요청 (POST /api/member/gyms)
    
    alert(`[${selectedGym.name}] 등록이 완료되었습니다!`);
    navigate("/"); // 대시보드로 복귀
  };
    ...
    <Box sx={{ bgcolor: "#f5f7fa", minHeight: "100vh", pb: 10 }}>
      ...
      <Container maxWidth="sm" sx={{ mt: 2 }}>
        <Stack spacing={2}>
          ...
          {/* 4. 검색 결과 목록 */}
          <Paper elevation={0} sx={{ borderRadius: 3, border: "1px solid #eef2f6", minHeight: 200 }}>
            {loading ? (
